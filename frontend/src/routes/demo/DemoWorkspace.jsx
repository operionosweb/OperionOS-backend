...
const sections = ["overview","clauses","obligations","deadlines","risks","evidence","assistant"];
...
export default function DemoWorkspace() {
  ...
  const content = {overview:<WorkspaceOverview contract={contract}/>,clauses:<Clauses contract={contract} getEvidence={getEvidence} openEvidence={setSelectedEvidence}/>,obligations:<Obligations contract={contract} getEvidence={getEvidence} openEvidence={setSelectedEvidence}/>,deadlines:<Deadlines contract={contract} getEvidence={getEvidence} openEvidence={setSelectedEvidence}/>,risks:<Risks contract={contract} getEvidence={getEvidence} openEvidence={setSelectedEvidence}/>,evidence:<EvidenceExplorer contract={contract} openEvidence={setSelectedEvidence}/>,assistant:<AssistantLanding contract={contract}/>}[activeSection];
  return <>
    ...
    {contract.aircraft && <Link className="od-aircraft-link" to="/demo/live-tracking"><Plane size={18}/><span><small>Associated aircraft / demo relationship</small><strong>{contract.aircraft.registration} · {contract.aircraft.model}</strong></span><ExternalLink size={15}/></Link>}
    ...
    <div className="od-workspace-content"><div className="od-section-title"><span className="od-eyebrow">{activeSection} intelligence</span><h2>{activeSection.charAt(0).toUpperCase()+activeSection.slice(1)}</h2><DemoBadge>DEMO DATA</DemoBadge></div>{content || <EmptyState title="No prepared data" description="This demonstration layer has no fixture records."/>}</div>
